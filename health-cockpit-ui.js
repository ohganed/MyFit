(() => {
  ...
  style.textContent = `
    .cockpit-entry{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 16px;margin-bottom:14px;background:#fff;border-radius:18px;box-shadow:0 2px 14px rgba(17,24,39,.05)}
    .cockpit-entry small{display:block;color:#6b7280;font-weight:700;margin-bottom:3px}.cockpit-entry strong{font-size:17px}.cockpit-entry button{white-space:nowrap}
  ...
  entry.className = 'cockpit-entry';
  entry.innerHTML = `<div><small>HEALTH</small><strong>Health Cockpit</strong></div><button class="ghost small" id="openCockpitBtn">Open</button>`;
    ...
    <div class="cockpit-shell">
      ...
      <div class="cockpit-grid">
        <section class="cockpit-card">
          <h3>Weight</h3><div class="latest" id="hcWeightLatest">—</div>
          <label>kg<input id="hcWeight" type="number" inputmode="decimal" step="0.1" min="0"></label>
          <button class="primary" id="hcSaveWeight">Save</button><div class="cockpit-status" id="hcWeightStatus"></div>
        </section>
        <section class="cockpit-card">
          <h3>Blood Pressure</h3><div class="latest" id="hcBpLatest">—</div>
          <div class="cockpit-two"><label>Systolic<input id="hcSys" type="number" inputmode="numeric" min="0"></label><label>Diastolic<input id="hcDia" type="number" inputmode="numeric" min="0"></label></div>
          <button class="primary" id="hcSaveBp">Save</button><div class="cockpit-status" id="hcBpStatus"></div>
        </section>
        <section class="cockpit-card">
          <h3>Sleep</h3><div class="latest" id="hcSleepLatest">—</div>
          <div class="cockpit-two"><label>Bedtime<input id="hcBed" type="datetime-local"></label><label>Wake<input id="hcWake" type="datetime-local"></label></div>
          <button class="primary" id="hcSaveSleep">Save</button><div class="cockpit-status" id="hcSleepStatus"></div>
        </section>
        <section class="cockpit-card">
          <h3>Energy</h3><div class="latest" id="hcEnergyLatest">—</div>
          <label>1–5<input id="hcEnergy" type="range" min="1" max="5" step="1" value="3"></label>
          <button class="primary" id="hcSaveEnergy">Save</button><div class="cockpit-status" id="hcEnergyStatus"></div>
        </section>
        <section class="cockpit-card cockpit-wide">
          <h3>Meal</h3><div class="latest" id="hcMealLatest">—</div>
          <label>Time<input id="hcMealTime" type="datetime-local"></label>
          <label>What did you eat?<textarea id="hcMealDesc" rows="2" placeholder="rice, grilled fish, vegetables"></textarea></label>
          <button class="primary" id="hcSaveMeal">Save Meal</button><div class="cockpit-status" id="hcMealStatus"></div>
          <div class="cockpit-mini-row"><span class="cockpit-chip">Nutrition detail: next</span><span class="cockpit-chip">Vitamins / Minerals ready in data model</span></div>
        </section>
      </div>
    </div>`;
  ...
  $('openCockpitBtn').onclick = async () => { overlay.hidden=false; document.body.style.overflow='hidden'; await refresh(); };
  ...
  window.MyFitHealthCockpitUI = { open: ()=>$('openCockpitBtn').click(), refresh };
})();
